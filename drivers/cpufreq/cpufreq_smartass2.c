 ...
 *
 * Author: Erasmux (modified for linux 3.x & SMP by imoseyon)
 *
 ...
 * also when suspended).
 */
#define DEFAULT_SLEEP_IDEAL_FREQ 350000
 ...
 */
//#define DEFAULT_RAMP_UP_STEP 290000
 ...
 */
//#define DEFAULT_RAMP_DOWN_STEP 360000
...
static int cpufreq_interactive_up_task(void *data)
{
        ...
        while (1) {
                ...
                for_each_cpu(cpu, &tmp_mask) {
                        ...
                        pcpu = &per_cpu(smartass_info, cpu);
                        smp_rmb();
                        ...
                        for_each_cpu(j, pcpu->cur_policy->cpus) {
                                struct smartass_info_s *pjcpu =
                                        &per_cpu(smartass_info, j);

                                if (pjcpu->ideal_speed > max_freq)
                                        max_freq = pjcpu->ideal_speed;
                        }

                        if (max_freq != pcpu->cur_policy->cur)
